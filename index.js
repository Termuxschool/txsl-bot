...
conn.sendMessage(id, 'Owner TXSL wa.me/+55 98 985254022' ,MessageType.text);
}
else if (text == 'affis'){
  ...
  var teks = text.replace(/#texthunder /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, '🔍 Processando...', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
    ...
    axios.get(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${bg}`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ ❗ ] Processando🔍 silahkan tunggu sebentar', MessageType.text, { quoted: m })
...
conn.sendMessage(id, 'Sucesso, Grupo Aberto💥' ,MessageType.text, { quoted: m } );
}
else if (text == '#closegc'){
 let hasil = `${id.split("@s.whatsapp.net")[0]}`;
   conn.groupSettingChange (hasil, GroupSettingChange.messageSend, true);
conn.sendMessage(id, 'Sucesso, Grupo Fechado💥' ,MessageType.text, { quoted: m } );
}
...
axios.get(`https://arugaz.herokuapp.com/api/howgay`).then((res) => {
	conn.sendMessage(id, '[📌] Monitorando...🔍', MessageType.text)
